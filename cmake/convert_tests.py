...
# Howto
#
# Run in the petsc source folder and pass the folder 'cmake' as argument
...
global petscSrcDir, numtotaltests, log, debug, binaries, current_folder, missing_sources, missing_execs, alltests, mfcont

# Set the directory you want to start from
petscSrcDir = os.getcwd()
datafiles = os.path.join("${PETSc_SOURCE_DIR}", "share", "petsc", "datafiles")
...
def processArgs(args, loopvars):
    ...
    args = args.replace("${PETSC_DIR}", petscSrcDir)
...
def parseTests(base,teststr,vars,forvars,curdir):
    ...
    for match in tests:
        ...
        if not os.path.isfile(os.path.join(petscSrcDir,curdir,outfile)):
            outfile = "FALSE"
...
if debug:
    ...
    log = open(logfile, 'w')
for dirName, subdirList, fileList in os.walk(petscSrcDir):
        ...
        if current_folder not in donefolders:
            ...
            if len(selected_tests) > 0:
                ...
                if m.group(3) == 'tests': 
                    ...
                    folders_with_tutorials.append(current_folder)
                # Find tests
...
includefile.write("# Tutorials\nif (BUILD_TUTORIALS)\n")
for fo in folders_with_tutorials:
    includefile.write("\tadd_subdirectory({})\n".format(fo))
